...
import com.piloto3.service.Piloto3ServiceTheharvester;

@RestController
public class Piloto3ControllerServiceTheharvester {

	String test = "";
	JSONObject jsonObject;

	@Autowired
	Piloto3ServiceTheharvester piloto3ServiceTheharvester;

	@Autowired
	MensajeriaTheharvester mensajeriaTheharvester;
	...
	ArrayList<MensajeriaTheharvester> allusers = new ArrayList<MensajeriaTheharvester>();

	@RequestMapping(value = "theharvester_service", method = RequestMethod.POST)
	public void doPostTheharvesterRouter(HttpServletRequest req, HttpServletResponse res) throws java.io.IOException {
		...
		if (test.length() == 10) { // el identificador desde la segunda llamada
			...
			if (this.metodoOperador(test, ping) == "no" || piloto3ServiceGeneral.testUrl(dataNet.getIp()) == "no") {
				...
				allusers.add(new MensajeriaTheharvester());
			...
			//	String command = "sudo docker run -t simonthomas/theharvester theharvester -d "+dataNet.getIp()+" -b "+dataNet.getCommand();
				String command = "sudo docker run -t simonthomas/theharvester theharvester "+dataNet.getIp();
				...
				service.submit(new Runnable() {
					public void run() {
						...
						piloto3ServiceTheharvester.engine(command, allusers.get(allusers.size() - 1), allusers);
	...
	public String metodoOperador(String test, String term) {///////////////////////////////////////////////////////////////////
		...
		mensajeriaTheharvester.setOperator(mensajeriaTheharvester); // manda el objeto para que opere desde el
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	public void metodoEnd(String id, ArrayList<MensajeriaTheharvester> users) { // finaliza el servicio
		...
		if (id != "-") {
			piloto3ServiceTheharvester.saleDbucle("1", id, users); // manda vaciar y eliminar el objeto
		}
