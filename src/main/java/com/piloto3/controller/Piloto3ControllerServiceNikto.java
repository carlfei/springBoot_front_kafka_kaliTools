...
import com.piloto3.model.MensajeriaNikto;

import com.piloto3.service.Piloto3ServiceGeneral;
import com.piloto3.service.Piloto3ServiceNikto;

@RestController
public class Piloto3ControllerServiceNikto {

	String test = "";
	JSONObject jsonObject;

	@Autowired
	Piloto3ServiceNikto piloto3ServiceNikto;

	@Autowired
	MensajeriaNikto mensajeriaNikto;
	...
	ArrayList<MensajeriaNikto> allusers = new ArrayList<MensajeriaNikto>();

	@RequestMapping(value = "nikto_service", method = RequestMethod.POST)
	public void doPostNiktorouter(HttpServletRequest req, HttpServletResponse res) throws java.io.IOException {
		...
		if (test.length() == 10) { // el identificador desde la segunda llamada
			...
			if (this.metodoOperador(test, ping) == "no" || piloto3ServiceGeneral.testUrl(dataNet.getIp()) == "no") {
				...
				allusers.add(new MensajeriaNikto());
			...
			//	String command = "ping -a 8.8.8.8";
				...
				// String command = "sudo docker run -t frapsoft/nikto -host " +dataNet.getIp();
				 String command = "sudo docker run -t frapsoft/nikto " +dataNet.getIp();
				// dataNet.getIp();
				// dataNet.getIp();
				...
				service.submit(new Runnable() {
					public void run() {
						...
						piloto3ServiceNikto.engine(command, allusers.get(allusers.size() - 1), allusers);
	...
	public String metodoOperador(String test, String term) {///////////////////////////////////////////////////////////////////
		...
		mensajeriaNikto.setOperator(mensajeriaNikto); // manda el objeto para que opere desde el
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	public void metodoEnd(String id, ArrayList<MensajeriaNikto> users) { // finaliza el servicio
		...
		if (id != "-") {
			piloto3ServiceNikto.saleDbucle("1", id, users); // manda vaciar y eliminar el objeto
		}
