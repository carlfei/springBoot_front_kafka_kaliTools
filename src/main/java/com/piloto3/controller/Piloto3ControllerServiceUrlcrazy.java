...
import com.piloto3.model.MensajeriaUrlcrazy;

import com.piloto3.service.Piloto3ServiceGeneral;
import com.piloto3.service.Piloto3ServiceUrlcrazy;

@RestController
public class Piloto3ControllerServiceUrlcrazy {

	String test = "";
	JSONObject jsonObject;

	@Autowired
	Piloto3ServiceUrlcrazy piloto3ServiceUrlcrazy;
	
	@Autowired
	MensajeriaUrlcrazy mensajeriaUrlcrazy;
	...
	ArrayList<MensajeriaUrlcrazy> allusers = new ArrayList<MensajeriaUrlcrazy>();

	@RequestMapping(value = "urlcrazy_service", method = RequestMethod.POST)
	public void doPostUrlcrazy(HttpServletRequest req, HttpServletResponse res) throws java.io.IOException {
		...
		if (test.length() == 10) { // el identificador desde la segunda llamada
			...
			if (this.metodoOperador(test, ping) == "no" || piloto3ServiceGeneral.testUrl(dataNet.getIp()) == "no") {
				...
				allusers.add(new MensajeriaUrlcrazy());
				...
				String command = "sudo docker run -t jamesmstone/urlcrazy:latest " +dataNet.getIp();
				...
				service.submit(new Runnable() {
					public void run() {
						...
						piloto3ServiceUrlcrazy.engine(command, allusers.get(allusers.size() - 1), allusers);
	...
	public String metodoOperador(String test, String term) {///////////////////////////////////////////////////////////////////
		...
		mensajeriaUrlcrazy.setOperator(mensajeriaUrlcrazy); // manda el objeto para que opere desde el
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	public void metodoEnd(String id, ArrayList<MensajeriaUrlcrazy> users) { // finaliza el servicio
		...
		if (id != "-") {
			piloto3ServiceUrlcrazy.saleDbucle("1", id, users); // manda vaciar y eliminar el objeto
		}
