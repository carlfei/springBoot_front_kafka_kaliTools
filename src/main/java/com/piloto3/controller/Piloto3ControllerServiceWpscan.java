...
import com.piloto3.model.MensajeriaWpscan;
import com.piloto3.service.Piloto3ServiceGeneral;
import com.piloto3.service.Piloto3ServiceWpscan;

@RestController
public class Piloto3ControllerServiceWpscan {

	String test = "";
	JSONObject jsonObject;

	@Autowired
	Piloto3ServiceWpscan piloto3ServiceWpscan;

	@Autowired
	MensajeriaWpscan mensajeriaWpscan;
	...
	ArrayList<MensajeriaWpscan> allusers = new ArrayList<MensajeriaWpscan>();

	@RequestMapping(value = "wpscan_service", method = RequestMethod.POST)
	public void doPostWpscan(HttpServletRequest req, HttpServletResponse res) throws java.io.IOException {
		...
		if (test.length() == 10) { // el identificador desde la segunda llamada
			...
			if (this.metodoOperador(test, ping) == "no" || piloto3ServiceGeneral.testUrl(dataNet.getIp()) == "no") {
				...
				allusers.add(new MensajeriaWpscan());
			...
			//	String command = "ping -a 8.8.8.8";
				...
				// command = "ping -a 8.8.8.8"; sudo docker run -t theyahya/sherlock user123
				 String command = "sudo docker run -t wpscanteam/wpscan " +dataNet.getIp();
				// dataNet.getIp();
				...
				service.submit(new Runnable() {
					public void run() {
						...
						piloto3ServiceWpscan.engine(command, allusers.get(allusers.size() - 1), allusers);
	...
	public String metodoOperador(String test, String term) {///////////////////////////////////////////////////////////////////
		...
		mensajeriaWpscan.setOperator(mensajeriaWpscan); // manda el objeto para que opere desde el
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	public void metodoEnd(String id, ArrayList<MensajeriaWpscan> users) { // finaliza el servicio
		...
		if (id != "-") {
			piloto3ServiceWpscan.saleDbucle("1", id, users); // manda vaciar y eliminar el objeto
		}
