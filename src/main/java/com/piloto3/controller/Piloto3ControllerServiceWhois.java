...
import com.piloto3.model.MensajeriaWhois;
import com.piloto3.service.Piloto3ServiceGeneral;

import com.piloto3.service.Piloto3ServiceWhois;

@RestController
public class Piloto3ControllerServiceWhois {

	String test = "";
	JSONObject jsonObject;

	@Autowired
	Piloto3ServiceWhois piloto3ServiceWhois;

	@Autowired
	MensajeriaWhois mensajeriaWhois;
	...
	ArrayList<MensajeriaWhois> allusers = new ArrayList<MensajeriaWhois>();

	@RequestMapping(value = "whois_service", method = RequestMethod.POST)
	public void doPostWhoisrouter(HttpServletRequest req, HttpServletResponse res) throws java.io.IOException {
		...
		if (test.length() == 10) { // el identificador desde la segunda llamada
			...
			if (this.metodoOperador(test, ping) == "no" || piloto3ServiceGeneral.testUrl(dataNet.getIp()) == "no") {
				...
				allusers.add(new MensajeriaWhois());
			...
			//	String command = "ping -a 8.8.8.8";
				...
				String command = "sudo docker run -t dentych/whois "+ dataNet.getIp();
				...
				service.submit(new Runnable() {
					public void run() {
						...
						piloto3ServiceWhois.engine(command, allusers.get(allusers.size() - 1), allusers);
	...
	public String metodoOperador(String test, String term) {///////////////////////////////////////////////////////////////////
		...
		mensajeriaWhois.setOperator(mensajeriaWhois); // manda el objeto para que opere desde el
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	public void metodoEnd(String id, ArrayList<MensajeriaWhois> users) { // finaliza el servicio
		...
		if (id != "-") {
			piloto3ServiceWhois.saleDbucle("1", id, users); // manda vaciar y eliminar el objeto
		}
