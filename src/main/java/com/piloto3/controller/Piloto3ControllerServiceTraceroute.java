...
import com.piloto3.model.MensajeriaTraceroute;

import com.piloto3.service.Piloto3ServiceGeneral;
import com.piloto3.service.Piloto3ServiceTraceroute;

@RestController
public class Piloto3ControllerServiceTraceroute {

	String test = "";
	JSONObject jsonObject;

	@Autowired
	Piloto3ServiceTraceroute piloto3ServiceTraceroute;

	@Autowired
	MensajeriaTraceroute mensajeriaTraceroute;
	...
	ArrayList<MensajeriaTraceroute> allusers = new ArrayList<MensajeriaTraceroute>();

	@RequestMapping(value = "traceroute_service", method = RequestMethod.POST)
	public void doPostTraceroute(HttpServletRequest req, HttpServletResponse res) throws java.io.IOException {
		...
		if (test.length() == 10) { // el identificador desde la segunda llamada
			...
			if (this.metodoOperador(test, ping) == "no" || piloto3ServiceGeneral.testUrl(dataNet.getIp()) == "no") {
				...
				allusers.add(new MensajeriaTraceroute());
				...
				String command = "sudo docker run -t gophernet/traceroute " + dataNet.getIp();
				...
				service.submit(new Runnable() {
					public void run() {
						...
						piloto3ServiceTraceroute.engine(command, allusers.get(allusers.size() - 1), allusers);
	...
	public String metodoOperador(String test, String term) {///////////////////////////////////////////////////////////////////
		...
		mensajeriaTraceroute.setOperator(mensajeriaTraceroute); // manda el objeto para que opere desde el
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	public void metodoEnd(String id, ArrayList<MensajeriaTraceroute> users) { // finaliza el servicio
		...
		if (id != "-") {
			piloto3ServiceTraceroute.saleDbucle("1", id, users); // manda vaciar y eliminar el objeto
		}
